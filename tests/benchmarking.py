...
def test_embedding_bge(benchmark):
    ...
    benchmark(embedder, "A study of the brightest 55 clusters ( Figure 3 ) ( Dunn Fabian 2006 , 2008 ) originally showed that over 70% of those clusters where the cooling time is less than 3 Gyr, and that therefore need heat, have bubbles; the remaining 30% have a central radio source.")


def test_embedding_bert(benchmark):
    embedder = encoder_embedder(
        model_name='bert-base-uncased', device=DEVICE, normalize=False)
    benchmark(embedder, ["A study of the brightest 55 clusters ( Figure 3 ) ( Dunn Fabian 2006 , 2008 ) originally showed that over 70% of those clusters where the cooling time is less than 3 Gyr, and that therefore need heat, have bubbles; the remaining 30% have a central radio source."])
