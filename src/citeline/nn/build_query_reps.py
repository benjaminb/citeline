...
"""
USAGE: python build_query_reps.py --config-path src/citeline/nn/configs/parquet_builds/small_test_build.yaml
...
def main():
    ...
    parser.add_argument("--using", type=str, help="Path to the dataset config YAML file.")
    args = parser.parse_args()
    path = Path(args.using)
    if not path.is_file():
        raise ValueError(f"Config file not found at {args.using}")
    build_dataset(path)
