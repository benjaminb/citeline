...
class H5DatasetWriter:
    ...
    def get_positives(self, df: pd.DataFrame, strategy: MultiSimilarityStrategy = None) -> np.ndarray:
        positives = strategy.rank_positives(
            queries=self.mapped_queries, dois=df["citation_dois"].tolist(), num_positives=self.num_positives
        )
        return positives

    def get_negatives(self, df: pd.DataFrame, strategy: MultiSimilarityStrategy = None) -> np.ndarray:
        negatives = strategy.rank_negatives(
            row=df, mapped_queries=self.mapped_queries, num_negatives=self.num_negatives
        )
        return negatives

    def write_h5(self, output_path: str):
        # Guarantee the output path exists
        output_dir = Path(output_path).parent
        output_dir.mkdir(parents=True, exist_ok=True)

        self.dataset["positives"] = self.get_positives(self.dataset, strategy=self.strategy)
        self.dataset["num_targets"] = np.array([len(pos_list) for pos_list in self.dataset["positives"]])
        self.dataset["negatives"] = self.get_negatives(self.dataset, strategy=self.strategy)

        # This explode causes us to have one anchor per query; multi-anchor approaches outside this project's scope
        export_df = self.dataset[["query_vectors", "positives", "negatives", "num_targets"]].explode(["query_vectors"])

        # Pad the postiives and negatives to ensure they have consistent shapes for saving to h5
